class Solution:
    ...
    def candy(self, A):
        ...
        data = {i: A[i] for i in range(n)}
        # neighbours of any value
        from collections import defaultdict
        n_map = defaultdict(list)
        for i in range(n):
            n_map[A[i]].append(i)

        val_map = sorted(data.items(), key=lambda kv: (kv[1], kv[0]))
        print val_map
        for idx in range(1, n):
            i, val = val_map[idx]
            j, prev_val = val_map[idx-1]
            # print ' i ', i, ' j ', j, ' val ', val  , ' prev ', prev_val
            print ' cur index ', idx, ' check neighbour value ', i, ' in ', n_map[i], ' does it have ', i -1, i + 1
            if val > prev_val and (i-1 in n_map[i] or i + 1 in n_map[i]):
